...
import com.fasterxml.jackson.core.json.UTF8JsonGeneratorAccess;
...
public class ReusableUTF8JsonGeneratorTest {
    ...
    public void canReuseAfterCloseAndReset() throws IOException {
        ...
        int expectedLength1 = UTF8JsonGeneratorAccess.outputTail(gen);
        ...
        int expectedLength2 = UTF8JsonGeneratorAccess.outputTail(gen);

        gen.close(); // writes into the buffer

        // then
        verify(os1, times(1)).write(eq(UTF8JsonGeneratorAccess.outputBuffer(gen)), eq(0), eq(expectedLength1));
        verify(os2, times(1)).write(eq(UTF8JsonGeneratorAccess.outputBuffer(gen)), eq(0), eq(expectedLength2));

        verify(delegate, atLeastOnce()).write(eq(UTF8JsonGeneratorAccess.outputBuffer(gen)), eq(0), eq(expectedLength1));
